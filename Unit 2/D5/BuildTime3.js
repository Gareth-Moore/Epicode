...
// Instructions
// 1. Search object strings for 'position' and 'location', search words needs to match both 'position' and 'location' to get a hit.
// 2. Search words can match any part of the string, e.g. 'US' returns a hit in 'California, US'
// 3. Search words must be case sensitive
// 4. Return only 'position' and 'location' whilst excluding 'description', 'requirements', 'benefits' and 'company_profile'
// 5. Return the total number of search hits.
// 6. Format of search results must look like this:
/* {
  results: [] - Assuming jobs.job_id is the requrested value, create an array of job_id values
  count: # - The total number of search hits
} */

console.log("Please type 'search()' and follow the prompts.")
...
function search() {
  ...
  // user input
  const user_input_title = prompt("Input job title:");
  const user_input_location = prompt("Input location:");

  // search algorithm
  jobs.forEach(value => {
    if (value.title.includes(user_input_title) && value.location.includes(user_input_location)) {
      searchResults.results.push(value.job_id);
      searchResults.count++;
    }
  });
